...
<div class="product-item">
	...
	<a class="product-item-image-wrapper" 
		...
		data-preload="false" 
		data-show-class="container" 
		data-custom-class="modal container" 
		data-src="<?=$item['DETAIL_PAGE_URL']?>?modal=Y"
	...
	<? endif; ?>
		...
		if ($item['SECOND_PICT'])
		{
			// dump($item['PREVIEW_PICTURE']);
			// dump($item['PREVIEW_PICTURE_SECOND']);

			$bgImage = !empty($item['PREVIEW_PICTURE_SECOND']) ? 
			CFile::ResizeImageGet($item['PREVIEW_PICTURE_SECOND']['ID'], array('width'=>400, 'height'=>400), BX_RESIZE_IMAGE_PROPORTIONAL_ALT, true)['src'] : 
...
<? endif; ?>
	<h3 class="product-item-title">
		<? if ($itemHasDetailUrl): ?>
		<a href="<?=$item['DETAIL_PAGE_URL']?>" title="<?=$productTitle?>" data-fancybox data-type="iframe">
			<? endif; ?>
			<?=$productTitle?>
			<? if ($itemHasDetailUrl): ?>
		</a>
