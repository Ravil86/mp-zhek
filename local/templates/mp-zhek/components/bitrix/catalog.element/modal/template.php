...
?>
<div class="container bx-catalog-element<?=$themeClass?>" id="<?=$itemIds['ID']?>" itemscope itemtype="http://schema.org/Product">
    <?php
	if ($arParams['DISPLAY_NAME'] === 'Y' || $_REQUEST['modal'] == 'Y')
	{
		?>
    ...
    <?php
		...
		?>
    <div class="div_r <?=($showBlockWithOffersAndProps ? "col-md-5 col-lg-6" : "col-md-5"); ?>">
        <div class="row">
            <?php
                                    ...
                                    <ul class="product-item-scu-item-list">
                                        <?php
																	foreach ($skuProperty['VALUES'] as &$value)
																	{
																		...
																		{
																			?>
                                        <li class="product-item-scu-item-color-container" title="<?=$value['NAME']?>" data-treevalue="<?=$propertyId?>_<?=$value['ID']?>"
                                        ...
                                        <?php
																		...
																		{
																			?>
                                        <li class="product-item-scu-item-text-container" title="<?=$value['NAME']?>" data-treevalue="<?=$propertyId?>_<?=$value['ID']?>"
                                            data-onevalue="<?=$value['ID']?>">
                ...
                <div class="product-item-detail-pay-block">
                    ...
                    <div class="mb-3">
                        <?php
										...
										{
											?>
                        <div class="product-item-detail-price-old mb-1" id="<?=$itemIds['OLD_PRICE_ID']?>" <?=($showDiscount ? '' : 'style="display: none;"')?>>
                        ...
                        <div id="<?=$itemIds['BASKET_ACTIONS_ID']?>" style="display: <?=($actualItem['CAN_BUY'] ? '' : 'none')?>;">
                            ...
                            <?php
											...
											if ($showBuyBtn)
											{
												?>
                            <div class="mb-3">
                                <a class="btn <?=$buyButtonClassName?> product-item-detail-buy-button more" id="<?=$itemIds['BUY_LINK']?>" <?//href="javascript:void(0);" ?>
                                    data-bs-toggle="collapse" href="#book-content" role="button" aria-expanded="false"
                                    >
                                    <?=$arParams['MESS_BTN_BUY']?>
                                </a>
                            </div>
                            <div id="book-content" class="collapse">
                                <?$APPLICATION->IncludeComponent(
														"zhek:form",
														"book",
														[
                ...
                <?php
							...
							foreach ($arResult['SKU_PROPS'] as $skuProperty)
							{
								...
								foreach ($skuProperty['VALUES'] as $value)
								{
									...
									{
										?>
                <div class="product-item-selected-scu product-item-selected-scu-color selected" title="<?=$value['NAME']?>" style="background-image: url('<?=$value['PICT']['SRC']?>'); display: none;"
                    data-sku-line="<?=$i?>" data-treevalue="<?=$propertyId?>_<?=$value['ID']?>" data-onevalue="<?=$value['ID']?>">
                </div>
                <?php
									}
									else
									{
										?>
                <div class="product-item-selected-scu product-item-selected-scu-text selected" title="<?=$value['NAME']?>" style="display: none;" data-sku-line="<?=$i?>"
