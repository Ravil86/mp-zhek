...
class Exportjson extends Export
{
    ...
    /**
     * Перебирает массив полей и возвращает строку даты для полей даты
     *
     * @param array $array
     *
     * @return array
        ...
        foreach ($array as &$item) {
            foreach ($item as &$v_field) {
                if (is_array($v_field)) {
                    ...
                    foreach ($v_field as &$v_item) {
                        if (is_object($v_item)) {
                            if (get_class($v_item) == ('Bitrix\Main\Type\DateTime' || 'Bitrix\Main\Type\Date')) {
                                $v_item = $v_item->toString();
                            }
                        }
                    }

                    unset($v_item, $v_field);
    ...
    protected function DataPreparation(array $param)
    {
        ...
        array_walk_recursive($data, [&$this, 'Cp1251UTF8']);
