...
class Ajax
{
    ...
    public function SendEmail()
    {
        ...
        foreach ($arr_file as $value) {
            $arr_id_files[] = \CFile::SaveFile(
                \CFile::MakeFileArray(iconv(LANG_CHARSET, mb_internal_encoding() . '//IGNORE', $value)),
                'tmp_bendersay_exportimport'
            );
        }
        $arEventFields = ['param' => (int)$filter_post['hl_id'], 'send_email' => $filter_post['email']];
        // Отправка
    ...
    public function ObrFormCSV()
    {
        ...
        $url_data_file = iconv(
            ...
            LANG_CHARSET . '//IGNORE',
            $this->request->getPost('url_data_file')
        );
        ...
        if ($method == 'POST') {
            ob_start();
            if (!empty($url_data_file) && $hl_id > 0 && !empty($export_type) && check_bitrix_sessid(
            ) && $export_count_row > 0) {
                    ...
                    if ($res_save_file) {
                        \CAdminMessage::ShowMessage([
                            'MESSAGE' => Loc::getMessage(
                                'BENDERSAY_EXPORTIMPORT_ERROR_FILE_SAVE'
                            ) . '<a href="' . $url_data_file . '">' . $url_data_file . '</a>'
                                . '<br>' . Loc::getMessage('BENDERSAY_EXPORTIMPORT_ERROR_FILE_SAVE_SEND')
                                . '<input type="text" value="' . Option::get(
                                    'bendersay.exportimport',
                                    'export_send_email'
                                ) . '"> '
                                . '<input type="submit" id="send_email" data-file="' . base64_encode(
                                    $url_data_file
                                ) . '" data-hl_id="' . $hl_id . '"'
                                . ' value="' . Loc::getMessage(
                                    'BENDERSAY_EXPORTIMPORT_ERROR_FILE_SAVE_SEND_BUTTON'
                                ) . '" class="adm-btn-save" >'
            ...
            if (!empty($url_data_file) && $hl_id > 0 && !empty($export_type) && check_bitrix_sessid(
                ...
                if ($export_type == 'export_hl') {
                    ...
                    if ($res_save_file) {
                        \CAdminMessage::ShowMessage([
                            'MESSAGE' => Loc::getMessage(
                                'BENDERSAY_EXPORTIMPORT_ERROR_FILE_SAVE_JSON'
                            ) . '<a href="' . $url_data_file . '"  download>' . $url_data_file . '</a>'
                                . '<br>' . Loc::getMessage('BENDERSAY_EXPORTIMPORT_ERROR_FILE_SAVE_SEND')
                                . '<input type="text" value="' . Option::get(
                                    'bendersay.exportimport',
                                    'export_send_email'
    ...
    public function ImportDataJSON()
    {
        ...
        if ($method == 'POST') {
            ob_start();
            if (!empty($param['url_data_file']) && $param['hl_id'] > 0 && !empty($param['export_type']) && check_bitrix_sessid(
            ) && $param['export_count_row'] > 0) {
                    ...
                    if ($arr_import['status'] === true) {
                        ...
                        if (!empty($data['error'])) {
                            file_put_contents(
                                ...
                                ) . '/upload/tmp_bendersay_exportimport/ImportLog.txt',
                    ...
                    if ($result['fields_count'] < $result['fields_all_count']) {
                        \CAdminMessage::ShowMessage([
                            'MESSAGE' => Loc::getMessage('BENDERSAY_EXPORTIMPORT_PROGRESS_BAR_IMPORT')
                                . ' ' . $result['fields_count'] . ' ' . Loc::getMessage(
                                    'BENDERSAY_EXPORTIMPORT_PROGRESS_BAR_IMPORT_IZ'
                                )
                        ...
                        if ($result['import_error_count'] > 0) {
                            \CAdminMessage::ShowMessage([
                                'MESSAGE' => str_replace(
                                    '#count#',
                                    $result['import_error_count'],
                                    Loc::getMessage('BENDERSAY_EXPORTIMPORT_ERROR_IMPORT_FILE')
                                )
    ...
    public function GetUserEntity()
    {
        ...
        if (!empty($res)) {
            foreach ($res as $value) {
                $result[$value['FIELD_NAME']] = iconv(
                    LANG_CHARSET,
                    'UTF-8//IGNORE',
                    $value['langs']['ru']['EDIT_FORM_LABEL'] . ' [<b>' . $value['FIELD_NAME'] . '</b>] (' . $value['ID'] . ')'
                );
            }
        }
     ...
     * @return string
     */
    public function GetUserEntityImport()
    {
     ...
     */
    public function ProverkaFields($arr)
    {
        if (is_array($arr)) {
            if (!array_key_exists('fields_name', $arr)) {
                throw new SystemException(
                    str_replace('#key#', 'fields_name', Loc::getMessage('BENDERSAY_EXPORTIMPORT_ERROR_NOT_KEY'))
                );
            }
            if (!array_key_exists('items_all_count', $arr)) {
                throw new SystemException(
                    str_replace('#key#', 'items_all_count', Loc::getMessage('BENDERSAY_EXPORTIMPORT_ERROR_NOT_KEY'))
                );
            }
            if (!array_key_exists('items', $arr)) {
                throw new SystemException(
                    str_replace('#key#', 'items', Loc::getMessage('BENDERSAY_EXPORTIMPORT_ERROR_NOT_KEY'))
                );
            }
    ...
    public function GetKey()
    {
        if ($this->request->getPost('export_type') == 'export_hl') {
            ...
            $str .= '<p>' . iconv(
                LANG_CHARSET,
                'UTF-8//IGNORE',
                Loc::getMessage('BENDERSAY_EXPORTIMPORT_ZAGLUSHKA')
            ) . '</p>';
            $str .= '</div>';
    ...
    public function ProvFileJSON()
    {
        ...
        if ($json === false || strlen($filename) == 0) {
            ob_start();
            \CAdminMessage::ShowMessage(
                iconv(LANG_CHARSET, 'UTF-8//IGNORE', GetMessage('BENDERSAY_EXPORTIMPORT_ERROR_GETUSERENTITYIMPORT'))
            );
            ...
            \CAdminMessage::ShowMessage(
                iconv(
                    ...
                    str_replace(
                        ...
                        GetMessage('BENDERSAY_EXPORTIMPORT_ERROR_FILE_SIZE')
                    )
                )
            );
